...
import com.snowplowanalytics.snowplow.eventgen.EventGen.strGen
...
object ContextsGen {
  ...
  val googlePrivateGen  =
    for {
      ...
      u  <- Gen.option(strGen(512, Gen.alphaNumChar)).withKeyNull("u")    // exceeds 256 on purpose
  ...
  val sessionContextGen =
    for {
      id   <- Gen.option(strGen(32, Gen.alphaNumChar)).withKey("id")
    } yield SelfDescribingData(
