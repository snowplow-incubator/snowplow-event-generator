...
object AnonymousIp extends SelfDescribingJsonGen {

  override def schemaKey: SchemaKey =
    SchemaKey("com.maxmind", "anonymous_ip", "jsonschema", SchemaVer.Full(1, 0, 0))

  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "is_anonymous"        -> genBool.optionalOrNull,
      "is_anonymous_vpn"    -> genBool.optionalOrNull,
      "is_hosting_provider" -> genBool.optionalOrNull,
      "is_public_proxy"     -> genBool.optionalOrNull,
      "is_tor_exit_node"    -> genBool.optionalOrNull
    )
}
