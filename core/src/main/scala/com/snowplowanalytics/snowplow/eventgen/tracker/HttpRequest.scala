...
final case class HttpRequest(method: Method, headers: Map[String, String], qs: Option[HttpRequestQuerystring], body: Option[HttpRequestBody])
...
object HttpRequest {
  ...
  def gen(eventPerPayloadMin: Int, eventPerPayloadMax: Int, now: Instant): Gen[HttpRequest] = genWithParts(HttpRequestQuerystring.gen(now), HttpRequestBody.gen(eventPerPayloadMin, eventPerPayloadMax, now))

  private def genWithParts(qsGen: Gen[HttpRequestQuerystring], bodyGen: Gen[HttpRequestBody]) = for {
    method <- Method.gen
    qs <- Gen.option(qsGen)
