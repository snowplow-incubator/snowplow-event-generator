...
object Conversion extends SelfDescribingJsonGen {

  override def schemaKey: SchemaKey =
    SchemaKey("io.snowplow.foundation", "conversion", "jsonschema", SchemaVer.Full(1, 0, 0))

  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "name"  -> strGen(1, 255).optional,
      "value" -> Gen.choose(0, 100000).optional
    )
