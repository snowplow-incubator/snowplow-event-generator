...
object PrivacySandboxTopics extends SelfDescribingJsonGen {

  override def schemaKey: SchemaKey =
    SchemaKey("com.google.privacysandbox", "topics", "jsonschema", SchemaVer.Full(1, 0, 0))
  ...
  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "topics" -> Gen.listOfN(5, topicEntryGen).optionalOrNull
    )
}
