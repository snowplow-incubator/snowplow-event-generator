...
import com.snowplowanalytics.snowplow.eventgen.protocol.event.{EventType, LegacyEvent, PagePing, PageView, UnstructEventWrapper}
...
object SdkEvent {
  private def extractWeb[A](el: Body, acc: Web => Option[A]): Option[A] = el.event match {
    case event: LegacyEvent => event match {
      case pp: PagePing => pp.deps.headOption.flatMap {
        case w: Web => acc(w)
        case _ => None
      }
      case pp: PageView => pp.deps.headOption.flatMap { w: Web => acc(w) }
      case _ => None
    }
    case _ => None
  }
  ...
  private def eventFromColPayload(p: CollectorPayload, fallbackEid: UUID): List[Event] = p.payload.map(el => Event(
    ...
    se_property = None,
    se_value = None,
    unstruct_event = el.event match {
      case UnstructEventWrapper(event, _) => event.toUnstructEvent
      case _ => UnstructEvent(data = None)
    },
    tr_orderid = None,
    tr_affiliation = None,
    tr_total = None,
