...
object ConsentDocument extends SelfDescribingJsonGen {
  ...
  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "id"          -> strGen(1, 36).required,
      "version"     -> strGen(1, 36).required,
      "name"        -> strGen(1, 60).required,
      "description" -> strGen(1, 1000).optional
    )
}
