...
object Alert extends SelfDescribingJsonGen {

  override def schemaKey: SchemaKey =
    SchemaKey("com.snowplowanalytics.monitoring.loader", "alert", "jsonschema", SchemaVer.Full(1, 0, 0))

  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "appName"    -> strGen(1, 64).required,
      "appVersion" -> strGen(1, 64).required,
      "message"    -> strGen(1, 200).required,
