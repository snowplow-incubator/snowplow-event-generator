...
object HttpRequestHeaders {
  ...
  def rawReqUriHeader(qs: Option[HttpRequestQuerystring]) = Map("Raw-Request-URI" -> List(Some("http://doesntmatt.er/p1/p2"), qs.map(_.toString)).flatten.mkString("?"))

  def genDefaultHeaders: Gen[Map[String, String]] = for {
  ...
  } yield Map("User-Agent" -> getV(ua), "Referer" -> getV(ref), ipName -> ip, "X-Forwarded-Proto" -> getV(xFwdProto), "Origin" -> getV(origin), "Cookie" -> getV(cookie), "SP-Anonymous" -> getV(anonymous)).filterNot { case (_, v) => v == "" } ++ FixedHeaders ++ custom

  private def genIpHeaderValue(headerName: String): Gen[String] =
    if (headerName != "X-Forwarded-For") genIp else genIps

  private def genIps = for {
    n <- Gen.chooseNum(1, 3)
    ips <- Gen.listOfN(n, genIp)
  } yield ips.mkString(",")

  private def genOrigin = for {
    n <- Gen.chooseNum(1, 3)
    os <- Gen.listOfN(n, genOriginHost)
  } yield os.map(getV).filterNot(_ == "").mkString(",")

  private def genCookie = for {
    n <- Gen.chooseNum(1, 5)
  ...
  } yield (ks.zip(vs).map { t => s"${t._1}=${t._2}" } ++ uuid.toList.map(c => s"_sp=${c.toString}")).mkString(";")

  private def genCustomHeaders: Gen[Map[String, String]] = for {
    n <- Gen.chooseNum(0, 10, 3, 5)
    customHeaders <- Gen.listOfN(n, genPair)
  } yield customHeaders.toMap

  private def genPair: Gen[(String, String)] = for {
    k <- genString("", 8)
    v <- genString("", 32)
  } yield (k, v)
