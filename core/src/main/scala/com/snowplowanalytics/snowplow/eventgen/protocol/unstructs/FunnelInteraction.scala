...
object FunnelInteraction extends SelfDescribingJsonGen {
  ...
  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "funnel_name"   -> strGen(1, 32).required,
      "step_name"     -> strGen(1, 32).required,
      "step_position" -> Gen.chooseNum(1, 100).required
    )
