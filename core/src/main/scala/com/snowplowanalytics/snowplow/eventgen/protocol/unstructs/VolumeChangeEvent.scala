...
object VolumeChangeEvent extends SelfDescribingJsonGen {

  override def schemaKey: SchemaKey =
    SchemaKey("com.snowplowanalytics.snowplow.media", "volume_change_event", "jsonschema", SchemaVer.Full(1, 0, 0))

  override def fieldGens(now: Instant): Map[String, Gen[Option[Json]]] =
    Map(
      "previousVolume" -> Gen.chooseNum(0, 100).optionalOrNull,
      "newVolume"      -> Gen.chooseNum(0, 100).required
    )
